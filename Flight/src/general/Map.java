...
import message.LinkedMessage;
import message.addParticleMessage;
...
public class Map {
	...
	void updateToon(Toon toon, ControlView control, Toon[] allToons) {
		updateToon(toon, control, allToons, new LinkedMessage());
	}

	void updateToon(Toon toon, ControlView control, Toon[] allToons,
			LinkedMessage changes) {
		attackUpdateToon(toon, control, allToons, changes);
		hookUpdateToon(toon, control, changes);
		accelerateUpdateToon(toon, control, changes);
		movementUpdateToon(toon, changes);
		// toon.setCubes();
		// control.outdate();
	}

	private void attackUpdateToon(Toon toon, ControlView control,
			Toon[] allToons, LinkedMessage changes) {
		// attacking old
		if (control.spaceAttack && toon.attackCur == 0) {
			toon.attackCur = toon.attackTime;
			changes.add(new addParticleMessage(addParticleMessage.EXPLOSION,
					toon.x, toon.y));
			for (Toon t : allToons)
				if (t.team != toon.team
		...
		if (control.mouseR && toon.attackCur == 0) {
			toon.attackCur = toon.attackTime;
			float arrowvx = control.worldMouse[0] - toon.x;
			float arrowvy = control.worldMouse[1] - toon.y;
			changes.add(new addParticleMessage(addParticleMessage.ARROW,
					toon.x, toon.y, arrowvx, arrowvy));
		}
	...
	private void hookUpdateToon(Toon toon, ControlView control,
			LinkedMessage changes) {
		// create hooking
		if (control.mouseL == 1) {
			float hookvx = control.worldMouse[0] - toon.x;
			float hookvy = control.worldMouse[1] - toon.y;
			Hook hook = new Hook(toon.x, toon.y, hookvx, hookvy, toon);
			changes.add(new addParticleMessage(addParticleMessage.HOOK, toon.x,
					toon.y, hookvx, hookvy, toon));
	...
	private void accelerateUpdateToon(Toon toon, ControlView control,
			LinkedMessage changes) {
		...
		if (control.up == 1 || control.up == 2) {
			if (toon.climbing)
				...
				toon.vy += toon.jetAcc;
				changes.add(new addParticleMessage(addParticleMessage.SMOKE,
						toon.x, toon.y, toon.vx, toon.vy));
			}
		}
	...
	private void movementUpdateToon(Toon toon, LinkedMessage changes) {
		toon.climbing = false;
		toon.grounded = false;
